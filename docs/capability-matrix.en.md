...
- Use `boss schema` as the source of truth: it currently exposes 39 top-level commands, with 15 first-level recruiter subcommands under `hr`, while `ai` and `resume` remain command-group entries.
